...
(define (accumulate-filter condition? operation null-value start end term next)
  (if(> start end)
     null-value
     (if(condition? start) 
        (operation (term start) (accumulate-filter condition? operation null-value (next start) end term next))
        (accumulate-filter condition? operation null-value (next start) end term next)))
)
