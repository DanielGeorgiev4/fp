...
(define after-2010 (filter (lambda (x) (> (caddr x) 2010)) movies))

; ratings трябва да е списък с рейтингите на всички филми, излезли след 2010г.
(define ratings (map (lambda (x) (cadr x)) after-2010))

; average-rating-after-2010 - среден рейтинг на филмите, излезли след 2010г.
(define (avg xs acc count)
    (if(null? xs)
       (/ acc count)
       (avg (cdr xs) (+ acc (car xs)) (+ count 1)))
)
(define average-rating-after-2010 (avg ratings 0 0))